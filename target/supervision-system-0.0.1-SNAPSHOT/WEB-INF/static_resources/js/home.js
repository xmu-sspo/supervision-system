...
function update_history(){
    ...
    if (history) {
        ...
        $.each(history,function(i,v){
            html += "<a class='btn btn-default' onclick='input_search(\""+v+"\")';' role='button'>"+v+"</a>"
        })
        $("#history").html(html);
    };
}
...
/**
 * [input_search 点击搜索历史，输入历史关键词到搜索输入框内]
 * @return {[type]} [description]
 */
function input_search(v){
    document.getElementById("keywords").value = v;
}
